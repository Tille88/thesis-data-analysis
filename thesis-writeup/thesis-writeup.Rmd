...
- Do legends increase the accuracy of decoding opacity-mapped values over knowing only the range of the data being mapped (i.e. when no legend is included)?
- How is decoding accuracy affected by legend designs that introduce more context from the base map or reduce the distance from legend to opacity-encoded data?
- Do subjects perceive  legend designs with more context from the base map as as more helpful in decoding opacity mapped values?
- How are decision times and user selection behaviour affected by legend choices? 
...
Using the web interface of \emph{View 2}, the users visually estimated the value at the location of a marker within the overlay data area, one example at a time.
...
There were no clear results indicating differences in response accuracy depending on the type of legend, and there was quite a large variation in response accuracies in the sample overall. These results, in combination with the significant results in that having a legend included reduces decoding errors, indicate that the decoding support from a legend helps create an reference point for the user, but that it is not very precise. Minor variations in presentation and context does not result in differences that are significant.
