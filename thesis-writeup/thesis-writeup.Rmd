...
<!-- TODO-FINAL-SUB: CHECK THAT page should start on an uneven page number on the right side of your open report. -->

\section{Introduction} \label{introduction}

## Opacity/transparency in GIS visualizations

Having a base map that is needed for geographic context and the need to visualize other geospatial data as an overlay layer on top of that geographic context results in the background being partly or fully concealed. To show part of the geographical context, a common method is to reduce the opacity of the overlay data layer. This is also sometimes referred to as increasing the inverse of opacity; transparency. 

@Ware20 highlights some issues of using low opacity layers in his seminal work on information visualization in relation to the use of transparency (alpha channel mapping/encoding) as a technique in information visualization. In particular, he describes that colours and objects that the user perceives become composites of the merged layers:
...
An illustrative example of how choosing lower opacity can show more of the base map, while higher opacity can make the geographic context difficult to see can be seen in Figure \ref{low-high-opacity-example}. With the low opacity, there is more of the base layer colour influencing the colour that is rendered, which in turn could make decoding the data more difficult.
...
\begin{figure}
     ...
     \begin{subfigure}[b]{0.6\textwidth}
         ...
         \caption{Opacity mapping 0.6-1.0 (fully opaque)}
...
This difficult decoding task can be made easier by interaction in web-based GIS systems. In those systems, a user can interact with the visualization to get values from for example tooltips on hover of the mouse over parts of the map. For static visualizations, that is not possible and on mobile views hovering effects are not possible. A legend is therefore often used both in interactive and static web-GIS visualizations.

This study aims to look at the efficiency of legend designs in decoding tasks. The study also investigates what legend designs presented to them the participants of the study percieve as most useful to the decoding task.

In order to design the study, the design of the enterprise GIS market leader Esri [@ArcgisIndex; @ArcgisPlatform] was together with the absence of a legend used as baselines in order to evaluate alternative legend designs.

ESRI's “ArcGIS Online” examples [@ArcgisBrightOnline; @ArcgisDarkOnline] using transparency are showing a legend that has a checkered background with white and mid-grey values as seen in figures \ref{fig:opacity-dark-background} and \ref{fig:opacity-bright-background}. This legend representation of transparency lacks any context of the visualization, and remains a checkered mid-grey background no matter the background of the base map. 
...
\begin{figure}
     ...
     \begin{subfigure}[b]{1.0\textwidth}
         ...
         \caption{Opacity data mapping and legend with colour scale, highlighted with a red bounding box [@ArcgisDarkOnline]}
    ...
    \vspace{0.4cm}
     ...
     \begin{subfigure}[b]{1.0\textwidth}
         ...
         \caption{Opacity data mapping and legend, highlighted with a red bounding box [@ArcgisBrightOnline]}
         \label{fig:opacity-bright-background}
     \end{subfigure}
        \caption{ArcGIS opacity legend and data mapping}
        \label{fig:arc-gis-opacity-legend-and-mapping}
\end{figure}
 

As will be discussed in section \ref{background}, legends have been studied, but not with focus on transparency decoding or attempts to use alternative techniques for transparency legends, such as contextualizing the legend or trying to increase the proximity of the legend to the encoded data. 

## Research questions

- Are legends increasing the accuracy of decoding opacity-mapped values compared to knowing only the range of the data being mapped?
- Does legends designs that introduces more context from the base map or reduces the distance from legend reduce decoding errors compared to Esri's ArcGIS's transparency legend design?
...
worldwide [@Veenendaal17]. According to a site measuring the presence of different web technologies online, maps was present in around 25% of the top 100,000 sites in July 2021, of which Google maps accounted for 74% [@Builtwith21]. In the list of frequently used map technologies there are also other flexible, and sometimes open source alternatives, such as MapBox, ArcGis and OpenStreetMap (often used with the Leaflet.js open source library). It is not easy to customize the base map that comes from centrally controlled servers, resulting in frequent use of colour scales and transparency in heat map visualizations.

![Online map technologies on top 100,000 websites, @Builtwith21](img/web_maps_top.png){width=50%}

This frequent exposure to maps creates a multiplying effect in the exposure of even less frequently seen map types. Lower opacity visualizations are suitable for visualizations of e.g. events such as typhoons/hurricanes. There doesn't seem like there exist many studies of the perceptions accuracy of such map designs as will be outlined in section \ref{literature}. This study focuses on the use of static legends in particular to be able to control the exposure to the treatment (different legend designs, or absense of legends).

\subsection{Previous related literature} \label{literature}

Related literature can be found in a few different research areas which intersects with the scope of this study.

In the field of perception and data visualization studies there is a long history on low-level studies measuring the ability of humans to correctly perceive different visual data encodings, such as position, length, angle, circle area, hue, etc. [@Cleveland828] and studies on perception effects from context, such as luminosity perception of a grey area depending on surrounding areas [@Adelson2042]. A more recent example of low-level perception focus can be found in @Rosenholtz07 attempt to find quantitative measurements of clutter in visual mediums.

For the higher-level tasks, @Tufte01 exemplifies attempts to bring these smaller encodings together to more stringently describe good data visualization practices. Muzner [-@brehmermuzner13] describes a gap between these low-level tasks, and high-level tasks in the data visualization literature.

In recent years, researches commonly attempts to bridge the gap towards other scientific areas [@Kim17; @Kim19], incorporating mental models, uncertainty and Bayesian prior beliefs of subjects into the perception task and to bridge the combination of the lower-level tasks and relate them to efficient user experiences through e.g. preattentive processing - meaning differentiating objects of interest to the viewer [@Treisman85; @Findlay98; @Wolfe04].

Transparency, and it's opposite (opacity) have been studied in the creation of mathematical transparency models [-@Metelli74] and recent literature heavily focused on computational rendering in a 3D/volume rendering context [e.g. @Chan09; and @Correa08].

Studies that cover transparency directly or indirectly for data visualization are more rare. @Jo19 briefly mentions opacity: "*To scale scatterplots, several approaches have been proposed, such as adaptive opacity [15, 30, 32] and aggregation [13, 53]. However, adaptive opacity does not scale well with the number of categories since multiple categorical colors become ambiguous when blended[..]"*.

For map and perception studies, colour choices in a cartographic context is a large corpus, with @Brewer06 having provided colour palette tools commonly used throughout data visualization from studying colour use from a cartographic/GIS perspective.

Combining explicit studies on transparency/opacity are rare. Some design of cartographic representations over as alternatives to choropleth maps have been making use of the alpha channel in order to preserve shape and topology in maps by using the alpha channel, instead of alternatives such as non-continuous cartograms that can alter shape/topology [@Roth10].

Targeted literature on legends are likewise scarce, and are commonly highly tied to different visualization types. In a walk-through of proper data visualization practices, Evergreen and Metzner suggests that there is a clear importance of data-proximity of legend usage and placement *"Because human eyesight has only a narrow range of focus, graphics should be placed very near their associated text (Malamed, 2009; Ware, 2012). [...] if a legend is required in the visualization, it should be so near the corresponding data points that no eye movements are needed to relate the two."* Direct labelling is seemed as preferable to legends altogether [@Evergreen13]. 

General legend design studies have been looking at legends that have are possible to interact with [@Riche10], but much of the literature is in the context of maps.

Recent map legend literature similarly have a clear design focus, where some have focused on designs that allow increasing interactivity with the legend [@Mitbo07: @Peterson99], adapting legends and placements to where users are gazing/looking [@Gobel18] and attempts to provide guidance on the use of map legends in a “dynamic environment” [@Dykes10]. More focused design studies have looked at fairly narrow questions related to legend designs for maps "cumulative frequency legends and diagrams" as a supplement to choropleth maps [@Cromley09; @Cromley06; @Chien19], and using a "frequency histogram legend" as a substitute to traditional legends for classed and un-classed choropleth maps [@Kumar04].

In a more holistic view on legend usage/design within web GIS, @Cybulski16 analyzed if animated maps that are published on the internet have designs that follow long-established guidelines within cartography, finding that they do follow the guidelines in general, but also that they are adapting to new technological and user circumstances.

No intersection between legends and transparency, either in GIS-context or elsewhere, have been found in the literature review. This is largely supported in a literature study that aimed to find whether there were cartographic rules for effective legend designs *"[...]it is found that only one study was dedicated to the building of cartographic rules for effective legend design"* [@Zhe17].
...
Due to the narrowness of the research questions, no openly available dataset or examples was likely to exist. This resulted in extensive work to design and create a dataset and a reproducible experiment where legend types was possible to isolate as the only non-randomly changing treatment.  
...
For more information on the process of example generations and technology choices see Appendix A.
 ...
 \begin{figure}
     ...
     \begin{subfigure}[b]{0.58\textwidth}
         ...
         \caption{Baseline - No legend ["Headline"], highlighted in blue}
         \label{fig:headline}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{0.58\textwidth}
         ...
         \caption{Baseline2 – ArcGIS Imitation ["Checkered"], highlighted in blue}
         \label{fig:checkered}
     \end{subfigure}
    \caption{Visualization baseline legend types}
    \label{fig:legend-types-baseline}
\end{figure}
...
\begin{figure}
     ...
     \begin{subfigure}[b]{0.58\textwidth}
         ...
         \caption{sampled context ["Sampled"], highlighted in blue}
         \label{fig:sampled}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{0.58\textwidth}
         ...
         \caption{clustered colour bands ["Clustered"], highlighted in blue}
         \label{fig:clustered}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{0.58\textwidth}
         ...
         \caption{Annotated Outline ["Annotated"], highlighted in blue}
         \label{fig:annotated}
     \end{subfigure}
        \caption{Visualization non-baseline legend types}
        \label{fig:legend-types-non-baseline}
\end{figure} 

**Baseline 1 - No legend ["Headline" in tables], (Figure \ref{fig:headline})**
...
**Baseline 2 – ArcGIS Legend Imitation ["Checkered" in tables], (Figure \ref{fig:checkered})**
...
This, and all following examples imitate the ArcGIS-design in the size of legend (constant for all legends). All legends except Annotated legend type were located in the bottom right corner.

 
**Legend with sampled context ["Sampled" in tables], (Figure \ref{fig:sampled})** 
...
The simplest design choice was to sample part of the base layer as background as seen in Figure \ref{fig:sampled}. The details on how this was done in practice can be found in Appendix A.
 
**Legend with clustered colour bands ["Clustered" in tables], (Figure \ref{fig:clustered})**

An alternative to sampling a rectangular section of the base layer map background, is to use the most common colours in the base layers as a background. In the design tested, the 10 most common colours of the background map was displayed as strips/columns in a vertical manner behind the data representation. This way they are contextualized for the full range of the data in the legend. More details how the common colours were identified are described in Appendix A. 

**Annotated Outline ["Annotated" in tables], (Figure \ref{fig:annotated})**
...
2. Randomly chosen images picked by the back-end server from the 315 example combination images\footnote{3 colours * 21 random data and marker location variations * 5 legend types = 315} stored in the back-end and presented to the respondents. Each response and a number of interactions by the respondents were sent back to the back-end server and persisted in a data base as the respondents went through the task. The response was picked using a number picker covering the range of the data (kept constant to 0-100). The initial value of the slider was random, and the respondents were required to change the response before being able to progress to the next example image. For more information on how the progression was generated can be found in Appendix B.
...
More information, example images of the user interface can be found in Appendix B.
...
*LEGEND* - Each of the legend types - the only part of visualization that is not randomized. Having values of Headline (no legend), Checkered, Clustered, Sampled and Annotated, corresponding to section \ref{legends}.
...
Each of the 34 respondent completed the full survey, resulting in 340 responses to the data-progression, and as seen they are distributed equally among the legend types (Table \ref{tab:responses-summary}).
...
```{r response-error-visualization, fig.pos = "h", out.width = '75%', fig.align = "center", fig.cap = "Perception error by visualization category", echo=FALSE}
...
As seen in Figure \@ref(fig:response-error-visualization), the mean is centered around 0, with some visually distinguishable differences in distribution between the Headline (no legend) category having the largest variance and more outliers compared to the examples with legends. The figure has the mean indicated by a black dot, with 1 standard deviation in each direction shown by a range-whisker for each category.
...
As seen by Figure \@ref(fig:abs-response-error-visualization), the same data visualized as boxplots with an absolute transformation of the perception error, shows the mean (visualized as a black point) differing between the categories - highest for Headline type, and lowest for Annotated legend type. As can also be seen there are some outliers\footnote{
defined for box plots as 1.5 times the interquartile  range [the height of the boxes] outside the interquartile range
} (visualized as dots outside above the "whiskers") for almost all categories, except for Headline, where the variance of the data is as quite large.


```{r abs-response-error-visualization, out.width = '75%', fig.align = "center", fig.cap = "Absolute error by visualization category", echo=FALSE}
...
*ACCEPTANCE* - objective valuation by participants on the usefulness of each legend type. On the scale 1-5, with responses of "No opinion" also allowed, and treated as missing values in the statistical modeling. 
...
stargazer(
  ...
  title="Acceptance response count cross-tab"
  )

```

Table \ref{tab:acceptancy-tally} indicates that the highest *ACCEPTANCE* scores are for Annotated and Sampled legend types. More statistical comparison to follow in the results section. 
...
*COLOUR* - Visualization presented having opacity data layer mapped colour channel as pure red, green or blue. From visual inspection, not any strong difference between the colours were perceptible (see Figure \@ref(fig:colour-visualization) in Appendix C). 
...
For *PERCEPTION ERROR* the average value is centered around 0, but the distribution variance is of interest. Most statistical models are constructed to compare in particular mean values between categories, which would not be able to give any indication of differences between these values. It would be possible to create a non-parametric test using bootstrapping of almost any test statistics [@Efron79]. However, for simplicity Bartlett's test \footnote{
**TODO - To ref and also not all shown** See https://en.wikipedia.org/wiki/Bartlett%27s_test and http://www.sthda.com/english/wiki/compare-multiple-sample-variances-in-r for comparisons of standard statistical test for variance differences
} allow testing of multiple variance differences between multiple groups.

The Bartlett's test will be conducted for both the full data including the Headline type as baseline, and a subset to see if variances differs in a statistically significant way between the different legend types, using Checkered legend type as a baseline, as the type having the largest variance test statistic of the sample.

Bartlett's test assumes normality, so the Shapiro-Wilk tests [@Shapiro65] are first conducted in order to see if the normality-assumption holds. This will be done for all legend types pooled, with Headline removed from the pooled sample, as well as for the subsample of each legend type separately.  

For *|PERCEPTION ERROR|*, *ACCEPTANCE* and other questions where a mean-comparison is able to be of relation to the research questions \footnote{
Including SUBMIT TIME, INPUT CHANGES and HOVER EVENTS, which will have the results presented in Appendix C
...
Controlling for time effects is considered to be added to the models to check for learning and/or fatigue effects as the respondent went through the progression of tasks. In order to see if this is of value to pursue further, first simple OLS-models are estimated to see if there are any indications of time effects.
...
The results from these test also feed into the possible need to do subsample analysis using only e.g. the first 5 responses of each respondent.

Subset analysis using only the respondents that are likely to be the most engaged or skilled, as defined as the respondents with below median average *|PERCEPTION ERROR|* is conducted to see if the results differ in a significant way for those respondents.
...
Table \@ref(tab:std-dev-by-cat) shows that the Headline types shows largest variance/standard deviation for the error, followed by Checkered, Sampled, Clustered and Annotated legend types.

In the statistical tests the Headline type is used first as baseline, to determine if these differences are statistically significant, then a subset removing all the Headline data points from the sample is used.
...
As seen from Table \@ref(tab:bartlett-test), there is a statistically significant difference in variance for the sample including the Headline type (p-val < 0.05), but that significance does not remain when excluding the Headline category. This indicates that there is not much need to look into two-way comparisons between the legend types.

For estimations of size of differences between groups transformation of *PERCEPTION ERROR* to absolute terms is analyzed as described in Section \ref{subsection-perception-error}.

\subsection{*|PERCEPTION ERROR|*} \label{subsection-perception-error}

Visual inspection of whether there are clear difference in heterogenity in *|PERCEPTION ERROR|* for both legend types is seen in Figure \@ref(fig:figure-heterogenity-legends) in Appendix C (bars including 95% confidence intervals). The confidence intervals are largely overlapping for all types except when not including any legend (Headline), which is in line with results from variance testing. 
...
The regression results shown in Table \@ref(tab:abs-perception-errors) show that the results are consistent with the variance tests, where model (1) estimates are relative to the Headline reference category. All other legend types display lower errors, and are all statistically significantly lower than the errors for Headline, with the Annotated type being having the lowest average error of `r round(baseline_legend_of_use$coefficients[[2]], 2)` lower errors than headline type on the range that is between 0-100.

Where Checkered is used as reference category (chosen due to both the research question formulation and the data distribution) the other designs are showing comparatively lower estimated errors, but none of those results are statistically significant.
...
Table \@ref(tab:ordered-acceptance) shows the mean for each legend type of the *ACCEPTANCE*-scores. As expected, having no legend at all (Headline) have the lowest acceptance scores, while the Sampled and Annotated legend types have average acceptance scores above 4 out of a maximum of 5.

Given the previous baseline categories still make sense, to check the statistical significance of these numbers compared to the Headline type, as well as against the Checkered type which imitates ArcGIS's legend choice.
...
As seen from the regression results in Table \@ref(tab:acceptance-models), all acceptance means are statistically higher than Headline category, and in comparison to the Checkered legend type, both Sampled and Annotated legend types are statistically significantly considered more helpful from the respondents. The Clustered legend type has a lower acceptance score compared to the Checkered legend, but not statistically significantly so.

The results were quite clear, and no strong reasons to believe that there are factors on individually controllable level that would alter the significance levels, no other models seemed necessary to control for these results.
...
Other variables that were collected as potential proxy variables for uncertainty (*SUBMIT TIME*, *INPUT CHANGES* and *HOVER EVENTS*), but exhibited high variability and large outliers to external factors did show some significant results as shown in Tables Table \@ref(tab:models-time-to-submit), \@ref(tab:models-input-changes) and  \@ref(tab:models-hover-events) in Appendix C. Many variables are significant compared to Headline legend types, but quite likely those outliers are coming from first interaction with the survey responses being for the headline type.
...
The results can be found in Table \@ref(tab:progression-models). As seen, neither the linear trend variable, nor the categorical variable for the last image in the progression were statistically significant. The estimate is even showing a slightly higher error for the last example on average compared to the first for the gathered sample of 68 (34 respondents, 2 examples per respondent).
...
For completeness, Table \@ref(tab:time-control-model) in Appendix C shows regression results for a model with Checkered reference category, with a linear progression variable included. The results of generally lower errors for other categories, but the earlier results of statistically significant estimates remains.

It is not of use to control for this with Headline-comparison models, as the Headline type was always the first and last example shown for each respondent by design.

Another robustness-test was introduced to see if there could be clear differing results between the samples with legends in the responses that were on average more accurate in their task - i.e. an attempt to remove outliers.
...
Visual inspection based on Figure \@ref(fig:models-outlier-removal-plot) shows that there is still a wide distribution of values, where Headline and Clustered legend types have the higher average absolute error, while the other types both have  a number of high error outliers.

```{r models-outlier-removal-plot, out.width = '75%', fig.align = "center", fig.cap = "Absolute error - 50th percentile most accurate respondent subset", echo=FALSE, message = FALSE}
# Absolute
...
As seen in Table \@ref(tab:abs-perception-errors-accurate), which is corresponding to Table \@ref(tab:abs-perception-errors) for this subsample, all estimates are showing statistically lower absolute errors compared to Headline category, except Clustered legend type, which is not any longer statistically significant. Checkered is still kept as reference category in the models where Headline types have been excluded, even though it is not the category with the lowest *|PERCEPTION ERROR|*. In conclusion, there is no difference in the significance after removing the less accurate respondents, with no statistically significant results.
...
In this study it has been investigated whether different legend design types can help in decoding transparency-mapped data in a web GIS visualization. A alternative focus has been on acceptance testing of the respondents to the different legend designs.

The results shows that in visualizations using transparency mapped overlays on top of a static base map, there is an influence of legend choices - as to if legends are included or not. It is evident that there is both a lower variance of errors in estimations from the respondents and more accurate results when there is a legend compared to when no legend (Headline type) is present. In other words, a legend makes visual comparisons possible and reduces estimation errors. 

As @Evergreen13 have pointed out there are other alternatives to legends for visualization such as simplicifaction of visualization, direct labeling of data, colour choices, altering visual weight and make use of motion. However, legends is such a common element for map visualization as shown by the relative prevalence of legend discussions in the map/GIS literature compared to the general data visualization literature.

There were no clear results indicating differences in response accuracy depending on the type of legend, and there was quite a large variation in response accuracies in the sample overall. Notably as well was that the number of times the user changed there resposne before submitting a response, or time until the user submitted the response from the visualization rendering were not well-suited to act as a proxy variables for respondent-confidence or uncertainty. During the data collection this may have been due to different respondents making use of the computer and UI-input elements in wastly different ways, and sometimes being distracted from the task or asking clarifying questions. 

If keeping the data in the same range between examples would lead the respondents to stop using the legends after getting used to/internalizing the data mapping. Without this the errors would not be on the same scale, so it was deemed more suitable, and instead a post-collection testing of the possible prevalence of progression effects was to be relied on in addition to changing the colours between each example. Given that there was a consistently higher error for the Headline examples, and actually slightly higher errors for the last example compared to the first, this was unlikely to have affected the validity of the study.

The most robust results were from the user preferences/acceptance of the different legend types, where both Sampled and Annotated (more contextualized and higher placement proximity to data) were preferred to the "industry standard" as in the imitation of the Checkered legend type. For the proximity type, this may be due to the reduced need for "attention switching" [@Kern10] due to higher proximity to the data and marker.
...
\begin{itemize}
  ...
  \item Only one opacity data layer is included. Multiple and possibly overlapping opacity layers would result in compounding opaqueness. However, this seems like a good design choice for data encoding to avoid more than one data opacity layer.
...
\subsection{Appendix A - example generations and technology choices}

A number of layers was produced for the sample images and extracted programmatically as PNG-files. The choices made can easily be described in combination with these layer types\footnote{All code available at https://github.com/Tille88/thesis-map-generation}.
...
In order not to have respondents making use of pre-existing knowledge or assumptions to the data given the common background of London, continuous data was generated using Perlin-noise [@Perlin85]\footnote{The implementation used to generate the noise was https://github.com/p5py/p5/blob/master/p5/pmath/rand.py }.
...
Two different types were generated: one with-out falloff to the edges of the data area, only using pure Perlin noise within the area (see Figure Figure \ref{fig:no-falloff}), and the other one making sure there was a more centralized higher data area, with decreasing values towards the edges of the rectangular data area (Figure \ref{fig:falloff}).
...
A marker was designed using vector-based SVG-API in the browser and overlaid at a randomized location within the data-extent. This was merged into the Canvas raster representation, and the value of the data later at the location was stored together with other metadata for each visualization to JSON (see Figure \ref{fig:marker}).



*Legends*

All legends were created using the Canvas API, in order to add text, background of legend and merge with data legend in a similar fashion as done between base map and data layer. Some specific points that are non-obvious described below:
...
**Clustered legend type** - Using kMeans clustering algorithm implementation in the base R language, was used to extract 10 "colour centers" representing common colours in the base map. These were then displayed as stripes behind the legend.
...
\subsection{Appendix B - user interface and data serving setup}

\subsubsection{Backend setup}

The backend logic used was quite simple: for each respondent/session, a identifier for the session is generated through a UUID, and a progression following the logic described in section \ref{data-collection-ui} was sent back to the front-end\footnote{All backend code can be found at https://github.com/Tille88/thesis-back-end}. 

The progression of the images started and ended with the Headline/no-legend type, and had the remaining 4 design type orders picked at random for the example progression images 2-5 and 6-9 respectively in order to maximize randomization. 
...
As an example: for the progression of images 1-10, three lists are generated: 

1. a colour progression ["green", "blue", "red]
2. a random list of numbers from the number of examples (21) [3, 14, 4, 10, 16, 2, 7, 9, 20]
3. a selection of legend types [[first 5: no-legend{fixed}, random order of 4 remaining types], last 5: random order of 4 remaining types, [no-legend{fixed}]]

Then these were combined as ["3-green-no-legend", "14-sampled-blue", "4-red-clustered", "10-green-annotated", ..., "20-green-no-legend"]

The front-end receiving the progression requested these images in turn from the equivalent of a file-server that contained the sample images.
...
All front-end UI was created with HTML5, CSS and basic Javascript, shown in a Chrome web browser\footnote{All frontend code can be found at https://github.com/Tille88/thesis-front-end}.

From the user-perspective, the user was first shown an introduction page giving some background and instructions as shown in the images in Figure \ref{fig:intro-page}.
...
\begin{figure}
     ...
     \begin{subfigure}[b]{0.55\textwidth}
         ...
         \caption{Introduction page start}
         \label{fig:elem-introduction}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{0.55\textwidth}
         \centering
     ...
     \begin{subfigure}[b]{0.55\textwidth}
         \centering
         \includegraphics[width=\textwidth]{img/intro3.png}
         \caption{Elements - data layer introduction}
         \label{fig:elem-data}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{0.55\textwidth}
     ...
     \begin{subfigure}[b]{0.55\textwidth}
         \centering
         \includegraphics[width=\textwidth]{img/intro5.png}
         \caption{Elements - marker introduction}
         \label{fig:elem-marker}
     \end{subfigure} 
     \begin{subfigure}[b]{0.55\textwidth}
         \centering
     ...
     \end{subfigure} 
        \caption{Introduction page progression}
        \label{fig:intro-page}
\end{figure}



Following the introduction, and a verbal check on understanding the task, the session was initialized, receiving the progression of examples chosen randomly from the backend (UI example shown in Figure \ref{fig:progression-ui}).
...
\begin{figure}
     ...
     \begin{subfigure}[b]{0.3\textwidth}
         \centering
         \includegraphics[width=\textwidth]{img/submit_inactive.png}
         \caption{Submit inactive}
         \label{fig:submit-inactive}
     \end{subfigure}
     \begin{subfigure}[b]{0.3\textwidth}
         \centering
         \includegraphics[width=\textwidth]{img/submit_active.png}
         \caption{Submit active}
...
Following the full 10 examples, the users were presented with acceptance questions as seen from images in Figure \ref{fig:acceptance-ui}.
...
All answers were sent back to the server and persisted. After final submission a message indicated the end of the session and task.


\newpage

\subsection{Appendix C - plots and tables}
...
<!-- TODO: INCLUDE OR NOT PLOT TYPE BELOW - IF REMOVE REFERENCE OTHER FIGURE! -->

```{r figure-heterogenity-legends, echo=FALSE, fig.cap = "Heterogeineity of absolute perception error across legend types"}
plotmeans(percept_error_abs ~ legend, main="Heterogeineity across legend types", data=no_legend_baseline, xlab = "Legend", ylab = "Absolute Perception Error")
```
...
```{r time-controlled-regression, echo=FALSE, message = FALSE}
time_controlled_reg <- lm(percept_error_abs ~ legend + prog,
                             data = checkered_baseline)

```

```{r time-controlled-regression-table, echo=FALSE, message = FALSE, results='asis'}
stargazer(
  time_controlled_reg,
  header = FALSE,
  type='latex',
  covariate.labels=c("Annotated","Clustered","Sampled", "Linear Progression", "Intercept/Checkered"),
  column.labels = c("Chekered Reference"),
  column.separate = c(1),
  dep.var.labels = c("|PERCEPTION ERROR|"),
  label = "tab:time-control-model",
  title="Absolute error progression linear trend with Clustered reference"
  )
