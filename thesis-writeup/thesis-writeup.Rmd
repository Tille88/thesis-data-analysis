...
This study aims to look at the efficiency of legend designs in visual opacity-to-value decoding tasks. The study also investigates which of 5 legend designs the participants of the study perceive as most helpful in the decoding task.

The legend design of the enterprise GIS market leader Esri [@ArcgisIndex; @ArcgisPlatform] was together with the absence of a legend used as baselines to evaluate alternative legend designs against.
...
As will be discussed in section \ref{background}, legends have been studied, but not with a focus on transparency decoding investigating alternative designs for transparency legends, such as contextualizing the legend or trying to increase the proximity of the legend to the opacity-encoded data. 

\subsection{Research questions} \label{introduction-res-questions}

- Can legends increase the accuracy of decoding opacity-mapped values compared to knowing only the range of the data being mapped?
- Does legend designs that introduce more context from the base map or reduce the distance from legend to opacity-encoded data decrease decoding errors?
- How are alternative legend design choices received by the subjects (subjective user acceptance testing)?
**TO BE DISCUSSED WITH SUPERVISOR: CAN BE ANSWERED WITH THIS DATA -> REMOVE?**
...
In the area of perception and data visualization studies, there is a long history of low-level studies measuring the ability of humans to correctly perceive visual data encodings, such as position, length, angle, circle area, hue, etc. [@Cleveland828]. There are also studies on perception effects from context, such as luminosity perception of a grey area depending on surrounding areas [@Adelson2042]. A more recent example of low-level perception focus can be found in @Rosenholtz07 attempt to find quantitative measurements of clutter in visual mediums. 

For higher-level tasks, @Tufte01 exemplifies attempts to bring these smaller encodings together to describe good data visualization practices. This corpus of literature usually tries to create design guidelines and best practices for data visualizations. 

These efforts to quantify design choices and data encoding methods have been criticized, where e.g. Muzner [-@brehmermuzner13] describes a gap between these low-level tasks and high-level tasks in the data visualization literature. In recent years, researchers commonly attempt to bridge the gap towards other scientific areas [@Kim17; @Kim19], incorporating mental models, uncertainty, and Bayesian prior beliefs of subjects into the perception task. The goal in this literature is to bridge the combination of the lower-level tasks and relate them to efficient user experiences through e.g. preattentive processing - meaning differentiating objects of interest to the viewer by giving it different encoding (e.g. giving data points of interest emphasis by encoding them in different colours than the other data points) [@Treisman85; @Findlay98; @Wolfe04].

Studies that cover transparency directly or indirectly for data visualization are rarer. @Jo19 briefly mentions opacity: "*To scale scatterplots, several approaches have been proposed, such as adaptive opacity [15, 30, 32] and aggregation [13, 53]. However, adaptive opacity does not scale well with the number of categories since multiple categorical colors become ambiguous when blended[..]"*.
...
The generation of the examples allowed all parts of the designs apart from the legends to be kept constant. This was done to control the exposure to the treatment (different legend designs, or absence of legends). The data layer used for opacity overlay was randomly generated.
...
The 5 legend design types are presented below, together with some justifications – where all legends and data mappings are linear in the opacity/alpha channel. This ought to introduce fewer complications than colour-mapping of colour due to a linear encoding of the alpha/opacity-channel in the RGBA-model\footnote{RGBA = Red, Green, Blue, Alpha}:
...
Only having a title indicating the range of values. Used to test the correctness of visual decoding by subjects without legend. Presented as the first and last example in the progression presented to the subjects.

**Baseline 2 – ArcGIS Legend Imitation ["Checkered" in tables], (Figure \ref{fig:checkered})**

The design is used as a baseline for models that explore if alternative legend choices are helping in decoding values compared to the current "industry standard". 

One likely downside making it more difficult for subjects to accurately decode the data values is the lack of background-context in the legend, i.e. the colours in the legend do not interact with a coloured background as it does in the visualization. The remaining legend design types are designed to combine the map base-layer background information into the legend in different ways.
...
The legend placement may heavily affect the users' ability to keep the information in near memory while moving the eyes back and forth between visualized data and the legend.

In an attempt to move the information closer to the data an outline legend was placed next to the data polygon. The legend is also contextualized by having the legend opacity superimposed over the base map layer.
...
All generated data were normalized to have the lowest value to be 0.0, and the highest being 1.0. To respondents of the survey, these values were shown to be in the range of 0-100.
...
All legends were created using the Canvas API, similarly as was done for the base map and data layer. The different legend types have been described in detail in section \ref{legends-design}.
...
The progression for each respondent was generated using a combination of colour, data example (data layer and marker location), and legend types using a simple algorithm\footnote{
...
*LEGEND* - categorical variable for each of the 5 legend types. The only part of visualization that is not randomized. Having values of Headline (no legend), Checkered, Clustered, Sampled, and Annotated, corresponding to section \ref{legends}.
...
*|PERCEPTION ERROR|* - absolute perception error. For linear modeling, there is a need to look at estimated mean differences between categories. Simply using *PERCEPTION ERROR* would result in all categorical differences being statistically insignificant unless the variance around 0 is heavily skewed.

*ACCEPTANCE* - objective valuation by participants on the usefulness of each legend type on a scale of 1-5. Opt-out responses of "No opinion" were allowed and treated as missing values in the statistical modeling. 

**Independent/Control Variables Candidates**

Three variables were collected in the hope to find a good proxy variable for uncertainty:

*SUBMIT TIME* - time in seconds from the rendering of the example until the subject submitted their response.

*INPUT CHANGES* - number of times the respondent changes their answer with input-slider before submitting. 

*HOVER EVENTS* - events of hovering the mouse cursor over the example image logged and sent back to the back end.

Two variables were collected as potential control variables:

*PROGRESSION* - for each respondent, the numbering of the examples in the progression from 1-10. Later to be used to check for learning and/or fatigue effects.
...
The parametric Bartlett's test \footnote{
...
} is used to test statistical differences between multiple variances for multiple groups. Bartlett's test will be conducted for both the full data including the Headline/no-legend type as a baseline and a subset to see if variances differ in a statistically significant way between the different legend types, using Checkered legend type as a baseline.

Due to that Bartlett's test assumes normality, Shapiro-Wilk tests [@Shapiro65] are first conducted to assert if the normality assumption holds. This will be done for all legend types pooled, with Headline removed from the pooled sample, as well as for the subsample of each legend type separately.  

For variables that are not centered around 0; *|PERCEPTION ERROR|*, *ACCEPTANCE*, *SUBMIT TIME*, *INPUT CHANGES* and *HOVER EVENTS* are examined using regular OLS-techniques (linear regression).
...
} (visualized as dots outside above the "whiskers") for almost all categories, except for Headline, where the variance of the *|PERCEPTION ERROR|* is quite large.
...
In the statistical tests, the Headline type was used first as a baseline, to determine if these differences are statistically significant, then a subset removing all the Headline data points from the sample was used.
...
As seen from Table \@ref(tab:bartlett-test), there is a statistically significant difference in the variance for the sample including the Headline type (p-value < 0.05), but that significance does not remain when excluding the Headline category.
...
The regression results in Table \@ref(tab:abs-perception-errors) show that the results are consistent with the variance tests, where model (1) estimates are relative to the Headline reference category. All other legend types display lower errors, and are all statistically significantly lower than the errors for Headline, with the Annotated type being having the lowest average error of `r round(baseline_legend_of_use$coefficients[[2]], 2)` lower errors than the Headline design on the range that is between 0-100.

In model (2), where Checkered is used as a reference category (chosen due to both the research question formulation and the data distribution) the other designs are showing comparatively lower estimated errors, but none of those results were statistically significant.
...
As seen in Table \@ref(tab:abs-perception-errors-accurate), which corresponds to Table \@ref(tab:abs-perception-errors) for the full sample, all estimates except for Clustered still have lower absolute errors compared to Headline category that are statistically significant. For comparison, Checkered is still kept as a reference category in the models where Headline types have been excluded from the sample. No statistically significant results were estimated for model (2), as was the case when using the full sample.
...
Feedback were given by some respondents was that it would be useful to have larger legends. The legend size in the study was chosen to imitate ArcGIS opacity legend as closely as possible and to be identical in size between legend types.
...
The sampling was using non-random convenience sampling. As outlined in section \ref{collection-info}, this was chosen to keep external factors, such as screen size and lighting, as consistent as possible. 
