...
- Can decision times and user selection behaviour be affected by legend choices? **TO BE DISCUSSED WITH SUPERVISOR: CAN BE ANSWERED WITH THIS DATA -> REMOVE? OR PERHAPS HIGHLIGHT MORE THAT LARGE OUTLIERS AND TOO NOISY DATA...**
...
Five different map legend designs were generated and then shown to respondents through a web user interface. Using this interface, the task for the respondents was to guess the value of the opacity-mapped overlay at the location of a marker.
...
As seen from Table \@ref(tab:bartlett-test), there is a statistically significant difference in the variance for the sample including the Headline type (p-value < 0.05), but that significance does not remain when excluding the Headline category from the sample.
