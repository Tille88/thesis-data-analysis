...
@Ware20 [p. 357] also includes a figure which shows a fully opaque data encoding on top of a map layer 
<!-- (Figure \ref{opaque-data}) -->
...
ESRI/ArcGIS examples and tutorials for their products, such as “ArcGIS online” and “ArcGIS Enterprise Map Viewer” are shown by the blog-entry “6 Easy Ways to Improve Your Maps”, explicitly listing the use of transparency. The way in which the transparency tool works is included as well 
<!-- [Figure \ref{fig:opacity-picker} -->
, @Berry2016] does not give the user much information or consider underlying layer colours and effects on perception.



This mapping has a similar visual representation to the end user of the maps as shown in the usage examples from the “ArcGIS Online” API documentation ArcGIS website (Figure \ref{fig:opacity-dark-background} [@ArcgisDarkOnline] and \ref{fig:opacity-bright-background} [@ArcgisBrightOnline]). Figure \ref{fig:opacity-dark-background} includes colour mapping, but the legend maps transparency into a checkered grey/white background, making it very difficult to decode the visual values. Figure \ref{fig:opacity-bright-background} has a single colour and a background layer corresponding roughly to the constant mid-grey and white checkered pattern.  
