...
Five different map legend designs were generated and tested through a web user interface. Using this interface, the task for the respondents were to guessing the value of the opacity-mapped overlay at the location of a marker.

The generation of the examples allowed all other parts of the designs to be kept constant. The data layer used for opacity overlay was randomly generated. 


\subsection{Legend design types} \label{legends}
...
\subsection{Legend design generation} \label{legend-generation}
...
\subsection{Data collection presentation and user interface} \label{data-collection-ui}


A front-end was developed using with HTML5, CSS and basic Javascript, shown in a Chrome web browser\footnote{All frontend code can be found at https://github.com/Tille88/thesis-front-end}. In this system, the respondents were presented with a progression of stages:

1. Introduction with instructions, introducing the elements of the visualization and tasks to guess/estimate the value of the data layer at the location of a randomly generated marker position making use of the different legend types (see Figure \ref{fig:intro-page}).
...
2. Following the introduction, and a verbal check on understanding the task, the session was initialized.

Randomly chosen images picked by the back-end server from the 315 example combination images\footnote{3 colours * 21 random data and marker location variations * 5 legend types = 315} stored in the back-end and presented to the respondents. 

The progression of the images started and ended with the baseline of Headline/no-legend type, and had the remaining 4 design type orders picked at random for the example progression images 2-5 and 6-9 respectively in order to maximize randomization. 
...
3. After progressing through the 10 example images, the respondents were asked about how helpful they considered the different legend types were for their task. This information was also sent back and persisted to the data base together with IDs of the respondents \ref{fig:acceptance-ui}.
...
\subsection{Data collection} \label{collection-info}
...
*COLOUR* - Visualization presented having opacity data layer mapped colour channel as pure red, green or blue. From visual inspection, not any strong difference between the colours were perceptible (see Figure \@ref(fig:colour-visualization) in Appendix A). 
...
Including SUBMIT TIME, INPUT CHANGES and HOVER EVENTS, which will have the results presented in Appendix A
...
Other variables that were collected as potential proxy variables for uncertainty (*SUBMIT TIME*, *INPUT CHANGES* and *HOVER EVENTS*), but exhibited high variability and large outliers to external factors did show some significant results as shown in Tables Table \@ref(tab:models-time-to-submit), \@ref(tab:models-input-changes) and  \@ref(tab:models-hover-events) in Appendix A. Many variables are significant compared to Headline legend types, but quite likely those outliers are coming from first interaction with the survey responses being for the headline type.
...
For completeness, Table \@ref(tab:time-control-model) in Appendix A shows regression results for a model with Checkered reference category, with a linear progression variable included. The results of generally lower errors for other categories, but the earlier results of statistically significant estimates remains.
