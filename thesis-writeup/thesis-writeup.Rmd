...
<!-- TODO: MAKE SURE PROGRESSION/TIME AND UUID(INDIVIDUAL) FE AS ROBUSTNESS, DO MANUALLY IF NEEDED, MOVE IN METHOD SECTION TO ROBUSTNESS NOT PART OF MAIN, CREATE OLS-REFERENCE INSTEAD -->
...
In the field of perception and data visualization studies is a long history on low-level studies measuring the ability of humans to correctly perceive different visual data encodings, such as position, length, angle, circle area, hue, etc. [@Cleveland828] and studies on perception effects from context, such as luminosity perception of a grey area depending on surrounding areas [@Adelson2042]. A more recent example of low-level perception focus can be found in Rosenholtz [-Rosenholtz07] attempt to find quantitative measurements of clutter in visual mediums.

For the higher-level tasks, Tufte [-@Tufte01] exemplifies attempts to bring these smaller encodings together to more stringently describe good data visualization practices. Muzner [-@brehmermuzner13] describes a gap between these low-level tasks, and high-level tasks in the data visualization literature.
...
Studies that cover transparency directly or indirectly for data visualization are more rare. Jo et al. [-@Jo19] briefly mentions opacity: "*To scale scatterplots, several approaches have been proposed, such as adaptive opacity [15, 30, 32] and aggregation [13, 53]. However, adaptive opacity does not scale well with the number of categories since multiple categorical colors become ambiguous when blended[..]"*.

For map and perception studies, colour choices in a cartographic context is a large literature, with Brewer [-@Brewer06] having provided colour palette tools commonly used throughout data visualization from studying colour use from a cartographic/GIS perspective.
...
In a more holistic view on legend within web GIS, Cybulski [-@Cybulski16] analyzed if animated maps that are published on the internet designed with guidelines that are given by cartographers, finding that they do follow the guidelines in general, but also that they are adapting to new technological and user circumstances.
...
To account for individual fixed effects\footnote{Certain respondents are likely to have different average correctness due to within-subject variations}, this is in case of statistically significant findings also tested using individual fixed effects (FE)-models. This can be thought of as including separate intercept-variables for each respondent using dummy variables for each respondent\footnote{The statistical analysis software may choose to estimate this using other techniques, such as entity-demeaned OLS for computational efficiency} [@Hanck20].

\begin{equation*}
|PERCEPTION ERROR|_{ij} = \alpha + \beta_{1} LEGEND_{i} + \gamma_{2} RES2{j} + \gamma_{3} RES3{j} + ... + \gamma_{n} RESn{j} + \varepsilon_{ij}
\end{equation*}

Where $RESn =$ RESPONDENTn 
...
# TODO: CLEAN UP -> ONLY OLS, ROBUSTNESS CHECK WITH TIME AND UUID DUMMIES...
# TBR <- with(no_legend_baseline,
#           data.frame(percept_error_abs = percept_error_abs - ave(percept_error_abs, uuid),
#                     legend = legend, uuid=uuid))
# summary(lm(percept_error_abs ~ legend - 1 + uuid, data = TBR))
# summary(TBR)
