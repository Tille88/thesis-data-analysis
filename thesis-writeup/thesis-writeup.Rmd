...
- Do subjects perceive  legend designs with more context from the base map as more helpful in decoding opacity mapped values?
...
There were no clear results indicating differences in response accuracy depending on the type/design of legend, and there was quite a large variation in response accuracies in the sample overall. These results, indicate that the decoding support from a legend helps create an reference point for the user, but that it is not very precise or sensitive to the design of the legend used. 

The design variations did not alter anything but the legend background and placement. Size of the legends and other elements, such as labels were kept constant. The legend size in the study was chosen to imitate ArcGIS opacity legend as closely as possible and to be identical in size between legend types. Feedback were given by some respondents that it would be useful to have larger legends.
