...
The noise data was put into the opacity value for a canvas image-element, with 1 of the 3 RGB-colour channels set to the max value (e.g. for red, canvas per that pixel was rgba(255, 0, 0, opacityValue)). The same data overlay mapped to the different colours are shown in Figure \ref{fig:map-colors}
...
*|PERCEPTION ERROR|* - As seen by Figure \@ref(fig:abs-response-error-visualization), the same data visualized as boxplots with an absolute transformation of the perception error, shows the mean (visualized as a black point) differing between the categories - highest for Headline type, and lowest for Annotated legend type. As can also be seen there are some outliers\footnote{
...
*ACCEPTANCE* - Table \ref{tab:acceptancy-tally} indicates that the highest *ACCEPTANCE* scores are for Annotated and Sampled legend types. More statistical comparison to follow in the results section.
...
*COLOUR* -From visual inspection, not any strong difference between the colours red/green/blue examples were perceptible (see Figure \@ref(fig:colour-visualization) in Appendix A). 
